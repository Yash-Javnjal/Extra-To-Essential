...
router.post('/', authenticateUser, ngoOnly, async (req, res) => {
  try {
    ...
    if (volunteer_id) {
      ...
      // ─── Email volunteer about the assignment (non-blocking) ───
      (async () => {
        try {
          // Get volunteer email
...
router.put('/:delivery_id/status', authenticateUser, ngoOnly, async (req, res) => {
  try {
    ...
    if (status === 'delivered') {
      ...
      // ─── Email donor + NGO about delivery completion (non-blocking) ───
      (async () => {
        try {
          // Get full delivery chain data
